...
avg_distance_complete_binary = matrix(NA, nrow = length(SCL), ncol = length(mm))
...
for (vv in 1:length(SCL)) {
  ...
  for (j in 1:rplct) {
    ...
    for (i in 1:length(mm)) {
      ...
      H = cbind(MM, X)
      ## estimator
      mu_hat <- (ginv(t(H)%*%H)%*%t(H)%*%YY)[1:8]
      ## rank estimator
      rank_hat <- rank(mu_hat)
      #print(rank_hat)
      ## distance
  ...
  ## 
  #distance_complete
  avg_distance_complete_binary[vv,] = colMeans(distance_complete)
  
}

## save data
#save(mm, avg_distance_complete_binary, file = "example2_rank_binary_cov_model_14.RData")

## plot
pdf(file = "example2_rank_binary_cov_model_14.PDF")
# plot against m
plot(mm, avg_distance_complete_binary[1,], type = 'l', pch = 18, col = 'black', 
     ylim = c(0, max(avg_distance_complete_binary)), 
     ylab="Correct ranking probability", xlab="m", 
     cex.main=1.25, cex.lab=1.5, cex.axis=1.1, lty = 2)

# Add a line for path graph
lines(mm, avg_distance_complete_binary[2,], type = 'l', pch = 18, col = "blue", 
      lty=2)

# Add a line for star graph
lines(mm, avg_distance_complete_binary[3,], type = 'l', pch = 18, col = "red", 
      lty=2)
